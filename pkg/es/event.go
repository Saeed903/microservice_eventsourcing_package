...
// Event s an internal representation of an event, returned when the Aggregate
// uses NewEvent to create a new evet. The events loaded from the db is
...
// GetMetadata is app-specific metadata such as request AggregateID, origination user etc.
...
func (e *Event) String() string {
	return fmt.Sprintf("(Event) AggregateID: %s, Version: %d, EventType: %s, AggregateType: %s, data: %s, Metadata: %s, Timestamp: %s, EventID: %s",
		e.AggregateID,
		e.Version,
		e.EventType,
		e.AggregateType,
